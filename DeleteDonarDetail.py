...
def delete_record():
    ...
    if email.get() == "":
        ...
        try:
            cur = conn.cursor()
            cur.execute("delete from Donar_detail where Emailid=?", (email.get(),))
...
title = Label(root_del, text="DELETE  DONAR  BY  EMAIL", font=("times new roman", 20, "bold"),foreground="white",bg="#f1053c")
