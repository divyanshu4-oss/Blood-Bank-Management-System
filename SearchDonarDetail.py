...
root_search.geometry("1199x600+100+50")        # set geometry
...
root_search.configure(bg="#f1053c")   # set bg color
...
def search_record():
    ...
    if fullname.get() == "":
        ...
        try:
            cur = conn.cursor()
            cur.execute("select * from Donar_detail where Fullname=?", (fullname.get(),))
...
title = Label(root_search, text="SEARCH  DONAR  DETAILS ", font=("times new roman", 20, "bold"), foreground="white",bg="#f1053c")
