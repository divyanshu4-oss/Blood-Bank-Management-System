...
menubar.add_cascade(label='BACK',command=home_page)  # add menu items to the File menu
    ...
    if email.get() == "" or mobile.get() == "" or fullname.get() == "" or bloodunit.get() == "":
        ...
        try:
            ...
            if row is None:
                ...
                cur.execute('update Donar_detail set Fullname = ?, MobileNo = ?, Blood_unit = ? where Emailid = ?',(fullname.get(),mobile.get(), bloodunit.get(),email.get()))
                conn.commit()
...
title =Label(root, text="UPDATE DONAR DETAILS", font=("times new roman", 20, "bold"), foreground="white",bg="#f1053c")
title.place(x=100, y=30)

# --------1st Row
l_email = Label(root, text="EMAIL ID", font=("times new roman", 15, "bold"), foreground="white",bg="#f1053c")
l_email.place(x=50, y=100)
txt_email =Entry(root, textvar=email, font=("times new roman", 15,))
txt_email.place(x=220, y=100, width=250)

# --------2nd Row
l_name = Label(root, text="FULL NAME", font=("times new roman", 15, "bold"), foreground="white",bg="#f1053c")
l_name.place(x=50, y=140)
txt_name = Entry(root, textvar=fullname, font=("times new roman", 15))
txt_name.place(x=220, y=140, width=250)

# ---------3rd Row
l_mobile = Label(root, text="MOBILE NO.", font=("times new roman", 15, "bold"), foreground="white",bg="#f1053c")
...
# ---------4th Row
l_blood = Label(root, text="BLOOD UNIT", font=("times new roman", 15, "bold"), foreground="white",bg="#f1053c")
...
# Button
b1 = Button(root, text="UPDATE",command=update_record, foreground="#f1053c", activebackground="orange",bg='white', width=10, height=1, font=("times new roman", 14, "bold")).place(x=280, y=266)
