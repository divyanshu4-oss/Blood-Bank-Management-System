...
def searh():
    root_viewall.destroy()
    import SearchAcceptorDetail
    reload(SearchAcceptorDetail)
def update():
    root_viewall.destroy()
    import Update_AcceptorDetail
    reload(Update_AcceptorDetail)
def delete():
    root_viewall.destroy()
    import DeleteAcceptorDetail
    reload(DeleteAcceptorDetail)
...
def viewall_record():
    ...
    try:
        cur = conn.cursor()
        cur.execute("select * from Receiver_detail")
...
#--------Button-------------------------->
b1 = Button(root_viewall, text="VIEW ALL RECEIVER DETAIL",command=viewall_record, foreground="#f1053c", activebackground="orange",bg='white', width=25, height=1, font=("times new roman", 14, "bold")).place(x=730, y=100)
